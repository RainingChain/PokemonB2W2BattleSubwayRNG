...
static std::map<u32, std::string> TrainerShortDescMap = []()
{
	...
	for (const json& trainer : trainers)
	{
		...
		std::string msgShort = msg.size() <= 20 ? msg : trainer["startMsg"].get<std::string>().substr(0, 20);
...
public:
	...
	{
		return shortDesc ? TrainerShortDescMap[id].c_str() : TrainerDescMap[id].c_str();
	}

	static const std::vector<trainerPokemon_t>& getAllTrainersPokemons()
	{
		static const auto allTrainersPokemons = ([]()->std::vector<trainerPokemon_t>
			{
