...
void BattleSubwayFilter::init() {
  ...
  for (const auto& playerPokemon : jsonPokemons) {
    ...
    if (!this->opts.playerPokemonsIdxFilter.empty() &&
        std::count(this->opts.playerPokemonsIdxFilter.begin(),
                   this->opts.playerPokemonsIdxFilter.end(), idx) == 0)
      continue;
...
bool BattleSubwayFilter::doesTrainerRespectFilter(
  ...
  return std::any_of(this->playerPokemons.begin(), this->playerPokemons.end(),
