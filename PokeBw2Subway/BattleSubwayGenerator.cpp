...
void BattleSubwayGenerator::advance(BWRNG& rng, u32 adv, const char* format, ...) const
{
	if (!this->isTraceActive())
	{
		rng.advance(adv);
	...
	for (u32 i = 0; i < adv; i++)
	{
		...
		std::cout << "\n" << frameAdvancedSum << " : ";
		frameAdvancedSum++;
		std::cout << "+" << advToPrint << " : " << "0x" << std::setfill('0') << std::setw(16) << std::hex << rng.getSeed() << std::resetiosflags(std::ios_base::basefield) << " : " << buffer << "";
...
const std::vector<u32>& getPossibleTrainersByWins(u32 wins, Subway type, u32 trainerIdxInBattle)
{
	//TODO: not correct list for superMulti, wifi, normalMulti

    if (type == SubwayType::normalSingle || type == SubwayType::normalDouble || type == SubwayType::normalMulti || type == SubwayType::wifi)
    {
        ...
        if (wins == 20) // 21
        {
			if (type == SubwayType::normalSingle)
				return list306_306;
			if (type == SubwayType::normalDouble)
				return list308_308;
			return list308_308;
        }
    }
    ...
    if (type == SubwayType::superSingle || type == SubwayType::superDouble)
    {
        ...
        if (wins == 48)
        {	
			if (type == SubwayType::superSingle)
				return list307_307;
			if (type == SubwayType::superDouble)
				return list309_309;
        }
        if (wins >= 49)
            return list200_299;
	...
	if (type == SubwayType::superMulti)
	{
		...
		if (wins >= 0 && wins <= 5)
			return list110_159;
		if (wins == 6 && trainerIdxInBattle == 1)
			return list160_199;
		if (wins >= 6 && wins <= 12)
			return list140_179;
		if (wins == 13 && trainerIdxInBattle == 0)
			return list160_199;
		if (wins == 13 && trainerIdxInBattle == 1)
			return list180_219;
		if (wins >= 14 && wins <= 19)
			return list160_199;
		if (wins >= 20 && wins <= 26)
		...
		if (wins >= 28 && wins <= 47)
			return list200_299;
		if (wins == 48)
			return trainerIdxInBattle == 0 ? list312_312 : list313_313;
		if (wins >= 49)
			return list200_299;
	}

    if (type == SubwayType::battleInstituteSingle || type == SubwayType::battleInstituteDouble)
    {
...
const auto& getTrainerBattleInstitute(u64 seed, const Container& Vector) {
    u32 divider = static_cast<u32>(std::ceil(static_cast<double>(0xffffffff) / Vector.size()));
    u32 v1 = seed >> 32;
...
void BattleSubwayGenerator::addPokemonsToTrainer(
	...
	bool forTeammate, const BattleSubwayTrainer* prevTrainerForMulti) const
{
    ...
    for (u32 i = 0; i < pokeCount;)
    {
		...
		this->log(" : Idx%d => %s", getIdx(rng.getSeed(), possiblePokemons), BattleSubwayData::pokeShortDesc(pokeIdToAdd));

		const bool isIncompatible = [&]() -> bool
		{
			const bool incompatibleWithOtherSelfPoke = std::any_of(trainerPokemons.data(), trainerPokemons.data() + i, 
			...
			if (forTeammate)
			{
				const auto& poke = BattleSubwayData::getAllTrainersPokemons()[pokeIdToAdd];
				const bool isIncompatibleWithPlayerPoke = poke.item == this->opts.playerTeam.item1
			...
			if (prevTrainerForMulti != nullptr)
			{
				...
				prevTrainerForMulti->forEachPokemon([&](const auto& prevTrainerPokemon, size_t)
					{
...
std::optional<BattleSubwayState> BattleSubwayGenerator::generate(u64 seed, u32 multiTeammateUnknownFrameAdvance) const
{
    BWRNG rng(seed);
    ...
    if (this->opts.subway.isSubway())
    {
		...
		if (this->opts.subway.isMulti())
		{
			...
			if (this->filter && !this->filter->doesMultiTeammateRespectFilter(state.getMultiTeammate()))
				return std::nullopt;
		}
		...
		for (u32 i = 0; i < battleCount; i++)
		{
			for (u32 j = 0; j < trainerCountByBattle;)
			{
				...
				const auto& possibleTrainers = getPossibleTrainersByWins(this->opts.wins + i, this->opts.subway, j);
        ...
        for (u32 i = 0; i < battleCount; i++)
        {
			for (u32 j = 0; j < trainerCountByBattle; j++)
			{
				...
				const auto prevTrainerForMulti = this->opts.subway.isMulti() && trainerIdx % 2 == 1 ? &trainers[trainerIdx - 1] : nullptr;
			...
			//after battle
			u32 pokeNpcCount = this->opts.subway.getPokemonCountByTrainer() * trainerCountByBattle;
			if (this->opts.subway.isMulti())
				pokeNpcCount += 2; // for teammate
			this->advance(rng, pokeNpcCount * 2, "unknown advances when starting battle");
    ...
    else
    {
        ...
        for (u32 i = 0; i < trainerCount; i++)
        {
            ...
            const auto& possibleTrainers = getPossibleTrainersByWins(this->opts.wins + i, this->opts.subway, 0);
			this->advance(rng, 1, "selectTrainer");
            const auto& trainerId = getTrainerBattleInstitute(rng.getSeed(), possibleTrainers);
            trainers[i].setTrainerId(trainerId);

            addPokemonsToTrainer(rng, trainer);
            if (this->filter != nullptr && !this->filter->doesTrainerRespectFilter(trainer))
                return std::nullopt;
			this->advance(rng, this->opts.subway.getPokemonCountByTrainer() * 2, "unknown advances when starting battle");
        }
    }
	if (this->isTraceActive())
		std::cout << "\n";
