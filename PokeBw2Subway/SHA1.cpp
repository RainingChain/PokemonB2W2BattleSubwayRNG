...
void SHA1::setTime(const Time &time, DSType dsType)
{
    u32 h = static_cast<u32>((bcd(time.hour()) + (time.hour() >= 12 && dsType != DSType::DS3 ? 0x40 : 0)) << 24);
    u32 m = static_cast<u32>(bcd(time.minute()) << 16);
    u32 s = static_cast<u32>(bcd(time.second()) << 8);
    u32 val = h | m | s;
    data[9] = val;
}
